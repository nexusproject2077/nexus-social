...
export default function SettingsPage({ user, setUser }) {
  ...
  const renderCreator = () => {
    const stats = [
      { label: t("followers"),       value: creatorStats?.followers_count, icon: "group" },
      { label: t("likes_received"),  value: creatorStats?.total_likes,     icon: "favorite" },
      { label: t("posts_label"),  value: creatorStats?.posts_count,     icon: "article" },
      ...
      <div className="flex min-h-screen" style={{ backgroundColor: C.surface }}>
        ...
        <main className="flex-1 overflow-y-auto">
          ...
          <div className="px-8 py-6 flex flex-wrap gap-x-6 gap-y-2 text-[10px] font-bold uppercase tracking-widest" style={{ borderTop: `1px solid rgba(255,255,255,0.04)`, color: C.outlineVar }}>
            <a href={`${API}/legal/terms-of-service`} target="_blank" rel="noopener noreferrer" className="hover:text-cyan-400 transition-colors">{t("terms")}</a>
            ...
            <a href={`${API}/legal/cookie-policy`} target="_blank" rel="noopener noreferrer" className="hover:text-cyan-400 transition-colors">{t("cookies")}</a>
