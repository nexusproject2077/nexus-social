...
export default function PrivacyCenter({ user, setUser }) {
  ...
  const loadPrivacySettings = async () => {
    try {
      const response = await axios.get(
        `${API}/gdpr/privacy/settings/${user.id}`,
      );
      setPrivacySettings(response.data);
    } catch (error) {
      console.error(t("error_loading_log"), error);
    }
  };
  ...
  const updatePrivacySettings = async (newSettings) => {
    try {
      await axios.put(
        `${API}/gdpr/privacy/settings?user_id=${user.id}`,
        newSettings,
      );
  ...
  const exportData = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `mes-donnees-${new Date().toISOString().split("T")[0]}.json`,
      );
  ...
  const requestDeletion = async () => {
    if (deleteConfirm !== t("DELETE_CONFIRM")) {
      alert('✗ Tapez t("DELETE_CONFIRM") pour confirmer');
      return;
    }
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            <div className="bg-red-950/20 border border-red-900/50 rounded-xl p-4">
              ...
              <div className="bg-red-950/30 border border-red-900/50 rounded-lg p-3 mb-3 text-xs text-slate-300">
                ...
                <p>
                  Toutes vos données seront supprimées définitivement après 30
                  jours. Vous pouvez annuler pendant ce délai.
                </p>
              </div>
              <button
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            <div className="bg-blue-950/20 border border-blue-900/50 rounded-xl p-4">
              ...
              <p className="text-xs text-slate-300">
                ...
                <strong className="text-white">dpo@nexussocial.com</strong>
              </p>
            </div>
            ...
            >
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={loading || deleteConfirm !== t("DELETE_CONFIRM")}
