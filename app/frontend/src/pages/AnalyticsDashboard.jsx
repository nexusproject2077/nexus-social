...
export default function AnalyticsDashboard({ user, setUser }) {
  ...
  const kpis = [
    { icon: FileText, color: "text-blue-500", label: t("posts_label"), value: stats?.total_posts, badge: stats?.posts_today ? `+${stats.posts_today}` : null },
    { icon: Heart, color: "text-red-500", label: t("likes_received"), value: stats?.total_likes, badge: null },
    { icon: MessageSquare, color: "text-purple-500", label: "Commentaires", value: stats?.total_comments, badge: null },
    { icon: Users, color: "text-cyan-500", label: t("followers"), value: stats?.followers_count, badge: stats?.new_followers_today ? `+${stats.new_followers_today}` : null },
