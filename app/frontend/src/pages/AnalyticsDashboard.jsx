...
export default function AnalyticsDashboard({ user, setUser }) {
  ...
  const loadAllData = async () => {
    try {
      ...
      console.error(t("error_loading_data_log"), error);
  ...
  const kpis = [
    {
      ...
      badge: stats?.posts_today ? `+${stats.posts_today}` : null,
    },
    {
      icon: Heart,
      color: "text-red-500",
    ...
    },
    {
      ...
      badge: stats?.new_followers_today
        ? `+${stats.new_followers_today}`
        : null,
    },
    {
      icon: Eye,
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <Card className="bg-slate-900 border-slate-800">
              <CardHeader>
                <CardTitle className="text-base sm:text-lg">
                  Activité de votre audience par heure
                </CardTitle>
                <CardDescription className="text-xs sm:text-sm">
                  Quand votre contenu reçoit des likes et commentaires (30
                  derniers jours)
                </CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <TabsContent value="trends" className="space-y-3 sm:space-y-4">
            <Card className="bg-slate-900 border-slate-800">
              <CardHeader>
                <CardTitle className="text-base sm:text-lg">
                  Votre croissance (30 derniers jours)
                </CardTitle>
                <CardDescription className="text-xs sm:text-sm">
          ...
          <TabsContent value="top" className="space-y-3 sm:space-y-4">
            <Card className="bg-slate-900 border-slate-800">
              <CardHeader>
                <CardTitle className="text-base sm:text-lg">
                  Vos meilleures publications
                </CardTitle>
                <CardDescription className="text-xs sm:text-sm">
                  Classées par engagement
                </CardDescription>
              </CardHeader>
              <CardContent>
                {topPosts.length === 0 ? (
                  <div className="text-center py-6 sm:py-8">
                    ...
                    <p className="text-slate-400 text-xs sm:text-sm">
                      Publiez du contenu pour voir vos meilleures performances
                      ici
                    </p>
                  </div>
                ) : (
                    ...
                    {topPosts.map((post, index) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 sm:gap-4 text-[10px] sm:text-xs text-slate-400 ml-8 sm:ml-11">
                          ...
                          <Badge
                            variant="secondary"
                            className="text-[9px] sm:text-xs ml-auto"
                          >
