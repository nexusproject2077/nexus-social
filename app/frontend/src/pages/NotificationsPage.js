...
const matchesTab = (n, tab) => {
  switch (tab) {
    ...
    case "likes":
      return ["like", "comment_like", "reaction"].includes(n.type);
    case "reposts":
      return ["repost", "share"].includes(n.type);
    case "follows":
      return ["follow", "follow_request", "follow_accepted"].includes(n.type);
    default:
      return true; // "all"
  }
};
...
const timeLabel = (iso, lang = "en") => {
  const bcp = BCP[(lang || "en").split("-")[0]] || "en-US";
  return new Date(iso).toLocaleTimeString(bcp, {
    hour: "2-digit",
    minute: "2-digit",
  });
};
...
export default function NotificationsPage({ user }) {
  ...
  const handleNotificationClick = async (notification) => {
    ...
    if (notification.type === "live" && notification.post_id) {
      ...
      ["follow", "follow_accepted", "follow_request"].includes(
        notification.type,
      )
    ) {
  ...
  const getIcon = (type) => {
    switch (type) {
      ...
      default:
        return <Heart className="w-4 h-4 text-slate-400" />;
    }
  };
  ...
  const getText = (n) => {
    switch (n.type) {
      ...
      case "comment":
        return n.comment_content
          ? `a commenté : « ${n.comment_content} »`
          : "a commenté votre publication";
      case "comment_like":
        return "a aimé votre commentaire";
      case "comment_reply":
        return n.comment_content
          ? `a répondu : « ${n.comment_content} »`
          : "a répondu à votre commentaire";
      case "follow":
        return "s'est abonné(e) à vous";
      case "repost":
        return "a reposté votre publication";
      case "mention":
        return "vous a mentionné dans une publication";
      case "trending":
        return "Votre publication est dans les tendances 🔥";
      case "reaction":
        return `a réagi ${n.comment_content || ""} à votre message`;
      case "follow_request":
        return "souhaite s'abonner à vous";
      case "follow_accepted":
        return "a accepté votre demande d'abonnement";
      case "live":
        return "est en direct 🔴 — rejoignez maintenant";
      case "moderation":
        return (
          n.comment_content ||
          "Un de vos contenus a été retiré par la modération."
        );
      case "tip":
        return n.comment_content || "vous a envoyé un pourboire 💸";
      default:
        return "";
    }
  };
        ...
        {loading ? (
          ...
          <div>
            {filtered.map((notif, i) => {
              ...
              const isFollow = ["follow", "follow_accepted"].includes(
                notif.type,
              );
              const canReply = ["comment", "comment_reply", "mention"].includes(
                notif.type,
              );
              return (
                <div key={notif.id}>
                  {showHeader && (
                    <div
                      className="px-4 py-1.5 text-xs font-bold uppercase tracking-wide sticky top-[42px] z-10 backdrop-blur-xl"
                      style={{
                        color: "#859397",
                        background: "rgba(11,19,38,0.85)",
                      }}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      >
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            navigate(`/profil/${notif.from_user_id}`);
                          }}
                          className="font-bold hover:underline"
                        >
                          {notif.from_username}
                        </button>{" "}
                        <span style={{ color: "#bbc9cd" }}>
                          {getText(notif)}
                        </span>
                      </p>
                      <p
                      ...
                      {notif.type === "follow_request" && (
                        <div className="flex gap-2 mt-2">
                          <button
                            ...
                            className="px-3 py-1 rounded-full text-xs font-bold hover:opacity-90"
                        ...
                        <div className="flex gap-2 mt-2">
                          {followedBack.has(notif.from_user_id) ? (
                            ...
                            <button
                              ...
                              className="px-3 py-1 rounded-full text-xs font-bold hover:opacity-90 flex items-center gap-1"
                              style={{ background: accent, color: "#00363e" }}
                            >
                              <UserPlus className="w-3.5 h-3.5" /> Suivre en
                              retour
                            </button>
                          )}
                        </div>
