...
const ephemeralLabel = (ttl, tFn = (k) => k) => {
  const opt =
    EPHEMERAL_OPTIONS.find((o) => o.ttl === Number(ttl)) ||
    EPHEMERAL_OPTIONS[0];
...
const imageSrcFromContent = (s) => {
  ...
  for (const { p, mime } of B64_IMAGE_SIGNATURES) {
    if (head.startsWith(p))
      return `data:image/${mime};base64,${s.replace(/\s/g, "")}`;
  }
  return null;
};
...
const formatDayLabel = (d, lang = "en", tFn = (k) => k) => {
  ...
  return date.toLocaleDateString(getBcp47(lang), {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
...
function MsgImage({ src, onOpen, onLoaded }) {
  ...
  if (failed) {
    return (
      <div
        className="rounded-xl mb-1 flex items-center gap-2 px-3 py-2 text-xs"
...
function VoiceMessage({ src, own }) {
  ...
  const bars = Array.from(
    { length: 28 },
    (_, i) => 30 + Math.abs(Math.sin(i * 1.3)) * 70,
  );

  return (
    <div
      className="flex items-center gap-2.5 mb-1 py-1"
      style={{ minWidth: 210 }}
    >
      <audio
        ref={audioRef}
...
function Ico({ name, size = 20 }) {
  ...
  switch (name) {
    ...
    case "mute":
      return (
        <svg {...p}>
          <path d="M6 8a6 6 0 0 1 9.4-5" />
          <path d="M18 8v5l2 3H4l2-3V8" />
    ...
    case "unmute":
      return (
        <svg {...p}>
          <path d="M18 8A6 6 0 0 0 6 8v5l-2 3h16l-2-3z" />
          <path d="M10 20a2 2 0 0 0 4 0" />
        </svg>
      );
    case "nickname":
    ...
    case "privacy":
      return (
        <svg {...p}>
          ...
          <path d="M9.4 12l1.8 1.8 3.4-3.7" />
        </svg>
      );
    case "group":
      return (
    ...
    case "palette":
      return (
        <svg {...p}>
          <path d="M12 3a9 9 0 1 0 0 18c1.4 0 2-1 2-2 0-1.6 1.2-2 2.5-2H18a3 3 0 0 0 3-3c0-5-4-9-9-9z" />
    ...
    case "timer":
      return (
        <svg {...p}>
          <circle cx="12" cy="13" r="8" strokeDasharray="3 2.4" />
          <path d="M12 13V9" />
          <path d="M9 3h6" />
        </svg>
      );
...
export default function MessagesPage({ user }) {
  ...
  const [conversations, setConversations] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("nexus_convs_cache") || "[]");
    } catch {
      return [];
    }
  });
  const [groups, setGroups] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("nexus_groups_cache") || "[]");
    } catch {
      return [];
    }
  });
  ...
  const [mutedIds, setMutedIds] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("nexus_muted") || "[]");
    } catch {
      return [];
    }
  });
  ...
  const toggleMute = (id) => {
    setMutedIds((prev) => {
      const next = prev.includes(id)
        ...
        : [...prev, id];
      try {
        localStorage.setItem("nexus_muted", JSON.stringify(next));
      } catch {}
  ...
  const onConvPullStart = (e) => {
    ...
    convPullStartY.current =
      c && c.scrollTop <= 0 ? e.touches[0].clientY : null;
  };
    ...
    try {
      ...
      mr.onstop = () => {
        ...
        if (blob.size > 4_000_000) {
          toast.error("Message vocal trop long");
          return;
        }
  ...
  const currentPic =
    selectedUser?.profile_pic || selectedGroup?.avatar_url || "";
  ...
  useEffect(() => {
    const onRealtime = (e) => {
      ...
      if (data.type === "reaction_update") {
        ...
        setMessages((prev) =>
          prev.map((x) =>
            x.id === ev.message_id ? { ...x, reactions: ev.reactions } : x,
          ),
        );
        return;
      }
      ...
      if (data.type === "ephemeral_changed") {
        const ev = data.data || {};
        if (selectedUserId && ev.peer_id === selectedUserId)
          setEphemeralTtl(ev.ttl_seconds || 0);
        return;
      }
  ...
  useEffect(() => {
    const mergeIfChanged = (incoming) => (prev) => {
      ...
      const changed =
        prev.length !== incoming.length || incoming.some((m) => !ids.has(m.id));
      ...
      try {
        if (selectedGroupId) {
          const r = await axios.get(
            `${API}/messages/groups/${selectedGroupId}/messages`,
          );
    ...
    const t = setTimeout(async () => {
      try {
        const res = await axios.get(
          `${API}/users/search?q=${encodeURIComponent(nmSearch)}`,
        );
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await axios.get(
          `${API}/users/search?q=${encodeURIComponent(addMemberSearch)}`,
        );
        const existing = new Set(groupMembers.map((m) => m.id));
        setAddMemberResults(
          (res.data || []).filter((u) => !existing.has(u.id)),
        );
      } catch {}
    }, 300);
    return () => clearTimeout(t);
  ...
  const fetchConversations = async () => {
    try {
      ...
      try {
        localStorage.setItem("nexus_convs_cache", JSON.stringify(list));
      } catch {}
    } catch {
      console.error("Erreur conversations");
    } // garde le cache en cas d'échec
  };
  ...
  const fetchGroups = async () => {
    try {
      ...
      setGroups(list);
      try {
        localStorage.setItem("nexus_groups_cache", JSON.stringify(list));
      } catch {}
  ...
  const fetchMessages = async (uid) => {
    try {
      ...
      try {
        const eph = await axios.get(
          `${API}/messages/conversations/${uid}/ephemeral`,
        );
  ...
  const setEphemeral = async (ttl) => {
    ...
    try {
      await axios.put(
        `${API}/messages/conversations/${selectedUserId}/ephemeral`,
        { ttl_seconds: ttl },
      );
      setEphemeralTtl(ttl);
      setShowEphemeralChooser(false);
      toast.success(
        ttl
          ? `${t("ephemeral_messages")} : ${ephemeralLabel(ttl, t)}`
          : `${t("ephemeral_messages")} désactivés`,
      );
    } catch {
      toast.error(t("error"));
    }
  ...
  const fetchGroupMessages = async (gid) => {
    try {
      ...
      toast.error(
        err.response?.data?.detail || "Erreur lors du chargement du groupe",
      );
      setMessages([]);
      setSelectedGroup(null);
    } finally {
    ...
    try {
      if (isGroup && selectedGroupId) {
        if (pendingImage || pendingAudio) {
          toast.error(t("media_unavailable_groups"));
          return;
        }
  ...
  const myReaction = (reactions) =>
    (Array.isArray(reactions) ? reactions : []).find(
      (r) => r.user_id === user.id,
    )?.emoji || null;
  ...
  const handleDeleteMessage = async (messageId, everyone = true) => {
    try {
      await axios.delete(`${API}/messages/${messageId}`, {
        data: { delete_for: everyone ? "everyone" : "me" },
      });
      setMessages((p) => p.filter((m) => m.id !== messageId));
      fetchConversations();
      toast.success(
        everyone ? t("message_deleted_everyone") : t("message_deleted_you"),
      );
    } catch {
  ...
  const translateMessage = async (msg) => {
    if (translations[msg.id]) {
      // déjà traduit → bascule (masque la traduction)
      setTranslations((t) => {
        const n = { ...t };
        delete n[msg.id];
        return n;
    ...
    try {
      const res = await axios.post(`${API}/messages/translate`, {
        text: msg.content,
        target,
      });
      if (res.data?.translated)
        setTranslations((t) => ({ ...t, [msg.id]: res.data.translated }));
  ...
  const forwardTo = async (target) => {
    ...
    try {
      if (target.kind === "group") {
        await axios.post(`${API}/messages/groups/${target.id}/messages`, {
          content: m.content || "",
        });
      } else {
  ...
  const renameGroup = async () => {
    ...
    if (!name) return;
    try {
      const res = await axios.put(`${API}/messages/groups/${selectedGroupId}`, {
        name,
      });
  ...
  const addGroupMember = async (u) => {
    try {
      await axios.post(`${API}/messages/groups/${selectedGroupId}/members`, {
        user_id: u.id,
      });
      setAddMemberSearch("");
      setAddMemberResults([]);
  ...
  const removeGroupMember = async (m) => {
    ...
    try {
      await axios.delete(
        `${API}/messages/groups/${selectedGroupId}/members/${m.id}`,
      );
      fetchGroupMembers(selectedGroupId);
  ...
  const toggleGroupAdmin = async (m) => {
    try {
      if (m.is_admin)
        await axios.delete(
          `${API}/messages/groups/${selectedGroupId}/admins/${m.id}`,
        );
      else
        await axios.post(`${API}/messages/groups/${selectedGroupId}/admins`, {
          user_id: m.id,
        });
      fetchGroupMembers(selectedGroupId);
  ...
  const startConversation = async () => {
    ...
    try {
      setLoading(true);
      const autoName = nmSelected
        .map((m) => m.username)
        .join(", ")
        .slice(0, 40);
      const res = await axios.post(`${API}/messages/groups`, {
        name: autoName,
        member_ids: nmSelected.map((m) => m.id),
      });
      if (res.data?.group) {
        setShowNewMessageModal(false);
        await fetchGroups();
  ...
  const handleClearConversation = async () => {
    ...
    try {
      ...
      setConversations((prev) =>
        prev.filter((c) => c.user_id !== selectedUserId),
      );
      setShowDetails(false);
      navigate("/messages");
  ...
  const getStatus = (msg) => {
    ...
    if (msg.status === "read")
      return <CheckCheck className="w-3 h-3" style={{ color: C.cyan }} />;
    if (msg.status === "delivered")
      return <CheckCheck className="w-3 h-3" style={{ color: C.outline }} />;
  ...
  const displayedMessages = convSearchQ
    ? messages.filter((m) =>
        (m.content || "").toLowerCase().includes(convSearchQ),
      )
    : messages;
  ...
  const lastOwnReadId = (() => {
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      if (m.sender_id === user.id)
        return m.status === "read" || m.read_at ? m.id : null;
    }
    return null;
  })();
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2">
              <p
                ...
                style={{
                  color: active ? C.cyan : C.onSurface,
              ...
              >
                ...
                {conv.pinned && (
                  <span
                    ...
                    style={{ fontSize: 13, color: C.outline }}
                  >
                    keep
                  </span>
                )}
            ...
            <p
              ...
              style={{
                color: unread ? C.onVariant : C.outline,
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p
              ...
              style={{ color: active ? "#a78bfa" : C.onSurface }}
            >
            ...
            <p className="text-xs truncate" style={{ color: C.outline }}>
              {group.last_message ||
                `${group.member_ids?.length || 0} ${t("members")}`}
            </p>
          </div>
        </button>
        {/* PC : 3 points au survol → menu (non lu / épingler / sourdine / supprimer). */}
  ...
  const ConvPanel = () => (
    ...
    >
      ...
      <div className="px-5 pt-5 pb-3 flex items-center gap-2">
        ...
        <h2
          className="font-black text-xl tracking-tight flex-1 text-center sm:text-left truncate"
          style={{
        ...
        <div className="relative">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (!e.target.value.trim()) setShowNewMsg(false);
      ...
      {!showNewMsg && (
        <div
          className="px-4 pb-4 pt-1 overflow-x-auto"
        ...
        >
          <div className="flex gap-5 items-end">
            ...
            >
              <div
                className="px-3 py-1.5 rounded-2xl text-[11px] font-medium leading-snug text-center shadow-lg"
                style={{
                  maxWidth: 86,
                  background: C.container,
                  color: myNote ? C.onSurface : C.outline,
              ...
              <div className="relative -mt-1">
                ...
                {!myNote && (
                  <div
                    className="absolute -bottom-0.5 -right-0.5 w-5 h-5 rounded-full flex items-center justify-center text-sm font-black"
                    style={{
                      background: C.cyan,
                      color: C.onPrimary,
                      border: `2px solid ${C.surface}`,
              ...
              <span
                className="text-[11px] font-semibold text-center truncate"
                style={{ width: 82, color: C.onSurface }}
              >
                Votre note
              </span>
            </button>
            ...
            {otherNotes.map((n) => (
              <button
                key={n.id}
                onClick={() => navigate(`/messages/${n.user_id}`)}
                className="flex flex-col items-center gap-1.5 flex-shrink-0"
                style={{ width: 88 }}
                title={n.content}
              >
                ...
                <span
                  ...
                  style={{ width: 82, color: C.outline }}
                >
                  @{n.username}
                </span>
              </button>
      ...
      <div
        ref={convScrollRef}
        onTouchStart={onConvPullStart}
        onTouchMove={onConvPullMove}
        onTouchEnd={onConvPullEnd}
        className="flex-1 min-h-0 overflow-y-auto pb-20 lg:pb-0"
      ...
      >
        ...
        {(convPull > 0 || convRefreshing) && (
          <div
            ...
            style={{
              height: convRefreshing ? 36 : convPull,
              transition: convPullStartY.current ? "none" : "height 0.2s",
            }}
          >
            <div
              ...
              style={{
                ...
                animation: convRefreshing
                  ? "spin 0.7s linear infinite"
                  : "none",
                transform: convRefreshing
                  ? "none"
                  : `rotate(${convPull * 4}deg)`,
                opacity: Math.min(1, convPull / PULL_THRESHOLD),
              }}
            />
          </div>
        )}
  ...
  const ChatPanel = () => (
    <div
      className={`flex-1 min-h-0 flex flex-col overflow-hidden ${hasSelection ? "flex" : "hidden sm:flex"}`}
    ...
    >
      {hasSelection && currentName ? (
        <>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <div>
                  ...
                  >
                    {isGroup
                      ? `${selectedGroup?.member_ids?.length || 0} ${t("members")}`
                      : t("encrypted_p2p")}
                  </p>
                </div>
              </button>
                  ...
                  <button
                    ...
                    onClick={() =>
                      window.dispatchEvent(
                        new CustomEvent("nexus:startcall", {
                          detail: {
          ...
          {!isGroup && ephemeralTtl > 0 && (
            ...
            >
              ...
              <span className="text-[11px] font-bold">
                {t("ephemeral_messages")} · {ephemeralLabel(ephemeralTtl, t)}
              </span>
            </div>
          )}
          ...
          >
            ...
            {(pullDist > 0 || refreshing) && (
              <div
                className="flex justify-center items-center overflow-hidden flex-shrink-0"
                style={{
                  height: refreshing ? 36 : pullDist,
                  transition: pullStartY.current ? "none" : "height 0.2s",
                }}
              >
                <div
                  className="w-6 h-6 rounded-full border-2"
            ...
            {loading ? (
              ...
              <div className="flex flex-col justify-center items-center h-full gap-3">
                <span
                  ...
                  style={{ color: C.outline, opacity: 0.3 }}
                >
                  forum
                </span>
                        ...
                        style={{
                          transform:
                            swipe.id === msg.id
                              ...
                              : "none",
                          transition:
                            swipe.id === msg.id ? "none" : "transform 0.2s",
                        }}
                      >
                        {/* Icône « répondre » révélée pendant le swipe */}
                        {swipe.id === msg.id && Math.abs(swipe.dx) > 12 && (
                          <span
                            className={`material-symbols-outlined absolute top-1/2 -translate-y-1/2 ${isOwn ? "right-full mr-1" : "left-full ml-1"}`}
                            style={{
                              color: C.cyan,
                              opacity: Math.min(
                                1,
                                Math.abs(swipe.dx) / SWIPE_REPLY,
                              ),
                            }}
                          >
                            reply
                        ...
                        <div className="relative max-w-[78%]">
                          ...
                          {repliedMsg && (
                            <div
                              className={`mb-1 px-2.5 py-1.5 rounded-xl border-l-2 ${isOwn ? "ml-auto" : ""}`}
                              style={{
                                background: C.high,
                                borderColor: C.cyan,
                                maxWidth: "100%",
                            ...
                            >
                              ...
                              >
                                {repliedMsg.sender_id === user.id
                                  ? "Vous"
                                  : repliedMsg.sender_username || ""}
                              </p>
                              <p
                                className="text-[11px] truncate"
                                style={{ color: C.onVariant, maxWidth: 220 }}
                              >
                                {isDataImage(repliedMsg.content) ||
                          ...
                          <div className="flex items-end gap-2">
                            ...
                            <div className="relative">
                              ...
                              {heartBurst === msg.id && (
                                <span
                                  className="absolute -top-3 left-1/2 -translate-x-1/2 text-2xl pointer-events-none z-20"
                                  style={{
                                    animation:
                                      "ping 0.6s cubic-bezier(0,0,0.2,1)",
                                  }}
                                >
                                  ❤️
                                </span>
                              ...
                              (on exclut le media_url si c'est en fait un audio). */}
                                ...
                                {translations[msg.id] && (
                                  <div
                                    className="mt-1.5 pt-1.5 text-sm"
                                    style={{
                              ...
                              {groupReactions(msg.reactions).length > 0 && (
                                <div className="flex flex-wrap gap-1 mt-1">
                                  {groupReactions(msg.reactions).map(
                                    ([emoji, count]) => {
                                      const mine =
                                        myReaction(msg.reactions) === emoji;
                                      return (
                                        <button
                                          key={emoji}
                                          onClick={() =>
                                          ...
                                          style={{
                                            background: mine
                                              ? `${C.cyan}22`
                                              : C.high,
                                            border: `1px solid ${mine ? C.cyan : C.outlineVar}`,
                                          }}
                                          title={
                                            mine
                                              ? "Retirer ma réaction"
                                ...
                                >
                                  ...
                                  {isOwn && (
                                    <button
                                      ...
                                      className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-red-500/20 hover:text-red-400 transition-colors"
                                      style={{ color: C.outline }}
                                    >
                                      <span className="material-symbols-outlined text-sm">
                                        delete
                              ...
                              {showEmojiPicker === msg.id && (
                                <div
                                  className={`absolute ${isOwn ? "right-0" : "left-0"} top-full mt-1 flex gap-1.5 px-3 py-2 rounded-2xl shadow-xl z-20`}
                                  style={glass}
                                >
                                  {QUICK_EMOJIS.map((e) => (
                                    <button
                          ...
                          {/* Status + time */}
                          <div
                            className={`flex items-center gap-1 mt-0.5 ${isOwn ? "justify-end" : "justify-start ml-9"}`}
                          >
                            {msg.expires_at && (
                              <span
                          ...
                          {!isGroup && msg.id === lastOwnReadId && (
                            <div className="flex justify-end mt-0.5">
                              ...
                              >
                                Vu
                                {msg.read_at
                                  ? ` ${new Date(msg.read_at).toLocaleTimeString(getBcp47(i18n?.resolvedLanguage || i18n?.language || "en"), { hour: "2-digit", minute: "2-digit" })}`
                                  : ""}
                              </span>
                            </div>
                          )}
          ...
          >
            ...
            {recording ? (
              ...
              >
                ...
                <span
                  className="material-symbols-outlined text-lg animate-pulse flex-shrink-0"
                  style={{ color: "#ef4444" }}
                >
                  mic
                </span>
                <span
                  ...
                  style={{ color: C.onSurface, minWidth: 42 }}
                >
                  {fmtSecs(recordSecs)}
                </span>
              ...
              >
                ...
                {!isGroup && (
                  ...
                  >
                    <span className="material-symbols-outlined text-sm">
                      {compressing ? "hourglass_top" : "image"}
                    </span>
                  </button>
                )}
                ...
                <button
                  type="submit"
                  disabled={
                    !messageContent.trim() && !pendingImage && !pendingAudio
                  }
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4">
          ...
          <button
            onClick={openNewMessage}
            className="px-5 py-2.5 rounded-xl font-bold text-sm transition-all active:scale-95 hover:opacity-90"
            style={{
              background: "linear-gradient(135deg,#22d3ee,#3b82f6)",
              color: C.onPrimary,
            }}
  ...
  const DetailsRow = ({ icon, label, sub, danger, onClick }) => (
    ...
    >
      ...
      <span className="flex-1 min-w-0">
        ...
        {sub && (
          <span className="text-xs block truncate" style={{ color: C.outline }}>
            {sub}
          </span>
        )}
      </span>
  ...
  const QuickAction = ({ icon, label, onClick }) => (
    ...
    >
      <span
        ...
        style={{ background: C.container, color: C.onSurface }}
      >
      ...
      <div className="flex flex-col h-full">
        ...
        >
          ...
          <div className="flex items-start gap-1 px-4 pb-5">
            ...
            />
            <QuickAction
              icon={isMuted(muteId) ? "unmute" : "mute"}
              label={
                isMuted(muteId) ? t("reactivate") : t("mute_notifications")
              }
          ...
          {isGroup ? (
            ...
            <>
                  ...
                  (editingName ? (
                    <div className="flex items-center gap-2 px-5 py-3">
                      ...
                      <button
                        ...
                        style={{ background: C.cyan, color: C.onPrimary }}
                      >
                        OK
                      </button>
                    </div>
              ...
              <div
                className="mt-2"
                style={{ borderTop: `1px solid ${C.outline}14` }}
              >
                <div className="flex items-center justify-between px-5 pt-3 pb-1">
                  ...
                  >
                    {groupMembers.length ||
                      selectedGroup?.member_ids?.length ||
                      0}{" "}
                    membres
                  </p>
                  {groupIsAdmin && (
            ...
            <div style={{ borderTop: `1px solid ${C.outline}14` }}>
              ...
              <DetailsRow
                icon="timer"
                label={t("ephemeral_messages")}
                sub={ephemeralLabel(ephemeralTtl, t)}
                onClick={() => {
                  setShowDetails(false);
                  setShowEphemeralChooser(true);
                }}
              />
              <DetailsRow
                icon="privacy"
                label={t("privacy_and_security")}
                onClick={() => {
                  setShowDetails(false);
                  navigate("/settings");
                }}
    ...
    <Layout user={user} compact hideMobileChrome bottomNav={!hasSelection}>
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        <div
          ...
          style={{
            ...
            left: "-5%",
            background:
              "radial-gradient(circle, rgba(34,211,238,0.04), transparent)",
          }}
        />
        <div
          className="absolute rounded-full blur-3xl"
          style={{
            ...
            right: "-5%",
            background:
              "radial-gradient(circle, rgba(59,130,246,0.04), transparent)",
          }}
        />
      </div>
      ...
      >
        ...
        {showDetails && hasSelection && (
          <div
            className="hidden lg:flex flex-col w-80 flex-shrink-0 border-l"
            style={{
              borderColor: "rgba(255,255,255,0.05)",
              background: `${C.surface}cc`,
            }}
      ...
      {convMenu && (
        <div
          className="fixed inset-0 z-[75] flex items-end sm:items-center justify-center select-none"
          style={{
        ...
        >
          ...
          >
            ...
            >
              ...
              />
              <p
                className="text-sm font-bold truncate max-w-[80%]"
                style={{ color: C.onSurface }}
              >
                {convMenu.name}
              </p>
            ...
            <div className="py-1">
              <button
                ...
                className="w-full flex items-center gap-4 px-6 py-3.5 transition-all hover:bg-white/5 text-left"
                style={{ color: C.onSurface }}
              >
                <span
              ...
              >
                ...
                <span className="text-[15px]">
                  {convMenu.pinned ? "Désépingler" : "Épingler"}
                </span>
              </button>
              <button
                onClick={handleToggleMutePref}
                className="w-full flex items-center gap-4 px-6 py-3.5 transition-all hover:bg-white/5 text-left"
                style={{ color: C.onSurface }}
              >
                <span
                ...
                <span className="text-[15px]">
                  {convMenu.muted
                    ? t("unmute_notifications")
                    : t("mute_notifications")}
                </span>
              </button>
              <button
                onClick={handleDeleteFromMenu}
                className="w-full flex items-center gap-4 px-6 py-3.5 transition-all hover:bg-red-500/10 text-left"
                style={{ color: "#f87171" }}
              >
                <span className="material-symbols-outlined">
                  {convMenu.kind === "group" ? "logout" : "delete"}
                </span>
                <span className="text-[15px]">
                  {convMenu.kind === "group"
                    ? "Quitter le groupe"
            ...
            <button
              onClick={() => setConvMenu(null)}
              className="w-full py-4 text-[15px] font-bold border-t"
              style={{ borderColor: C.outlineVar, color: C.outline }}
            >
              {t("cancel")}
            </button>
          </div>
        ...
        >
          ...
          >
                ...
                {QUICK_EMOJIS.map((e) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        background: mine ? `${C.cyan}22` : C.container,
                        border: mine
                          ? `2px solid ${C.cyan}`
                          : "2px solid transparent",
                      }}
            ...
            >
              ...
              {!isDataImage(msgMenu.content) &&
                (msgMenu.content || "").trim() && (
                  <>
                    <button
                      onClick={() => {
                        const m = msgMenu;
              ...
              <button
                onClick={() => {
                  handleDeleteMessage(msgMenu.id, false);
                  setMsgMenu(null);
                }}
                className="w-full flex items-center gap-4 px-6 py-3.5 transition-all hover:bg-white/5 text-left"
                style={{ color: C.onSurface }}
              ...
              {msgMenu.sender_id === user.id && (
                <button
                  onClick={() => {
                    handleDeleteMessage(msgMenu.id, true);
                    setMsgMenu(null);
                  }}
                  className="w-full flex items-center gap-4 px-6 py-3.5 transition-all hover:bg-red-500/10 text-left"
                  style={{ color: "#f87171" }}
      ...
      {forwardMsg && (
        <div
          className="fixed inset-0 z-[76] flex items-end sm:items-center justify-center"
        ...
        >
          ...
          >
              ...
              {chatItems.length === 0 ? (
                ...
                chatItems.map((item) => {
                  ...
                  return (
                    <button
                      key={item.key}
                      onClick={() => forwardTo({ kind: item.kind, id })}
            ...
            <button
              onClick={() => setForwardMsg(null)}
              className="w-full py-4 text-[15px] font-bold border-t flex-shrink-0"
              style={{ borderColor: C.outlineVar, color: C.outline }}
            >
              {t("cancel")}
            </button>
        ...
        >
          ...
          >
            ...
            <div className="flex justify-center mb-5">
              ...
              >
                <div
                  className="absolute top-0 px-3 py-1 rounded-full text-[11px] text-center max-w-[160px] truncate"
                  style={{
                    background: C.container,
                    color: C.onSurface,
                    border: `1px solid ${C.cyan}22`,
            ...
            <p className="text-[10px] mt-2" style={{ color: C.outline }}>
              Votre note disparaît après 24 h.
            </p>
      ...
      {showDetails && hasSelection && (
        <div
          className="lg:hidden fixed inset-0 z-[85] flex items-end"
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 mb-1 px-1">
              ...
              >
                {t("ephemeral_messages")}
              </h3>
            </div>
            <p className="text-xs px-1 mb-4" style={{ color: C.outline }}>
              Les nouveaux messages disparaissent automatiquement après la durée
              choisie, des deux côtés.
            </p>
        ...
        >
          ...
          >
            <div
              ...
              style={{ borderBottom: `1px solid ${C.outline}18` }}
            >
              <button
                onClick={() => setShowNewMessageModal(false)}
                style={{ color: C.outline }}
            ...
            >
              {nmResults.length === 0 ? (
                ...
                >
                  {nmSearch.trim()
                    ? "Aucun utilisateur"
                    : "Recherchez des personnes à contacter"}
                </p>
              ) : (
                nmResults.map((u) => {
                  const checked = Boolean(
                    nmSelected.find((x) => x.id === u.id),
                  );
                  return (
                    <button
                      key={u.id}
                      onClick={() => toggleNmSelect(u)}
            ...
            <div className="p-4 flex-shrink-0">
              <button
                ...
                disabled={nmSelected.length === 0 || loading}
              ...
              >
                {loading
                  ? "..."
                  : nmSelected.length >= 2
                    ? `Créer le groupe (${nmSelected.length})`
                    : "Discuter"}
              </button>
            </div>
          </div>
          ...
          >
            ...
            {groupSearchRes.length > 0 && (
              <div
                ...
                style={{ border: `1px solid ${C.outlineVar}` }}
              >
