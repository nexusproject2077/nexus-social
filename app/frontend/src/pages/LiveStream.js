...
export default function LiveStream({ user }) {
  ...
  useEffect(() => {
    ...
    if (sp.get("gift_sent")) { toast.success(t("gift_sent")); window.history.replaceState({}, "", window.location.pathname); }
  ...
  const start = async () => {
    ...
    if (isHost) {
      ...
      catch { toast.error(t("camera_mic_unavailable")); setStatus("idle"); return; }
      ...
      try { await axios.post(`${API}/live/start`, { room_id: roomId }); startedRef.current = true; toast.success(t("you_are_live")); }
      catch { /* best-effort */ }
    }
  ...
  const copyLink = () => {
    ...
    navigator.clipboard?.writeText(link).then(() => toast.success(t("link_copied")), () => toast.error("Copie impossible"));
  };
  ...
  if (status === "idle" || status === "ended") {
    return (
      <div className="fixed inset-0 flex flex-col items-center justify-center gap-5 px-6 select-none" style={{ background: C.surface, color: C.onSurface }}>
        ...
        <h1 className="text-2xl font-black" style={{ fontFamily: "Space Grotesk, sans-serif" }}>{isHost ? t("live") : "Rejoindre le direct"}</h1>
        <p className="text-sm text-center max-w-xs" style={{ color: C.outline }}>
          {status === "ended" ? t("live_ended") : (isHost ? t("live_features_desc") : "Vous allez rejoindre ce direct.")}
        </p>
