...
const GIFTS = [
  ...
  { emoji: "🎉", name: "Confetti", cents: 299 },
  { emoji: "🔥", name: "Feu", cents: 399 },
  { emoji: "💎", name: "Diamant", cents: 499 },
  { emoji: "👑", name: "Couronne", cents: 999 },
  { emoji: "🦄", name: "Licorne", cents: 1499 },
  { emoji: "🚀", name: "Fusée", cents: 1999 },
];
const euro = (c) =>
...
export default function LiveStream({ user }) {
  ...
  useEffect(() => {
    ...
    if (sp.get("gift_sent")) {
      toast.success(t("gift_sent"));
      window.history.replaceState({}, "", window.location.pathname);
    } else if (sp.get("gift_cancel")) {
      window.history.replaceState({}, "", window.location.pathname);
    }
    axios
      .get(`${API}/billing/status`)
      .then((r) => setPaidGifts(!!r.data?.enabled))
      .catch(() => {});
    if (isHost)
      axios
  ...
  const onboardCreator = async () => {
    try {
      ...
      toast.error("Configuration des paiements indisponible");
    }
  };
  ...
  const spawnGift = (g, from) => {
    ...
    setTimeout(
      () => setFlyGifts((arr) => arr.filter((x) => x.id !== id)),
      3200,
    );
  };
  ...
  const sendLike = () => {
    setLikes((n) => n + 1);
    spawnHeart();
    send({ type: "like", from: user.username });
  };
  const sendChat = () => {
  ...
  const sendGift = async (g) => {
    ...
    if (paidGifts) {
      try {
        const r = await axios.post(`${API}/live/gift-checkout`, {
          name: g.name,
          emoji: g.emoji,
  ...
  const finishRecordingAndSave = async () => {
    ...
    if (!window.confirm(t("save_replay_clips"))) return;
    setSavingReplay(true);
    try {
      const form = new FormData();
      form.append(
        "file",
        new File([blob], `replay_${Date.now()}.webm`, { type: "video/webm" }),
      );
      form.append("caption", "Replay du direct 🔴");
      await axios.post(`${API}/clips`, form, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      toast.success(t("replay_published"));
    } catch {
      toast.error("Impossible d'enregistrer le replay");
    } finally {
  ...
  const start = async () => {
    ...
    if (isHost) {
      ...
      try {
        ...
        toast.error(t("camera_mic_unavailable"));
        setStatus("idle");
        return;
      }
      ...
      try {
        ...
        outStream = cam; /* fallback sans filtre */
      }
    ...
    wsRef.current = ws;
    ws.onopen = () => {
      send(isHost ? { type: "hello-host" } : { type: "ready" });
      if (!isHost) setStatus("live");
    };
    ...
    ws.onmessage = async (event) => {
      ...
      if (msg.type === "gift") {
        spawnGift({ emoji: msg.emoji, name: msg.name }, msg.from);
        return;
      }
      if (msg.type === "viewers") {
    ...
    ws.onerror = () => toast.error(t("live_connection_failed"));
  };
  ...
  if (status === "idle" || status === "ended") {
    return (
      <div
        className="fixed inset-0 flex flex-col items-center justify-center gap-5 px-6 select-none"
      ...
      >
        ...
        >
          {isHost ? t("live") : "Rejoindre le direct"}
        </h1>
        <p
        ...
        >
          {status === "ended"
            ? t("live_ended")
            : isHost
              ? t("live_features_desc")
              : "Vous allez rejoindre ce direct."}
        </p>
        {savingReplay && (
        ...
        {isHost && connect?.enabled && (
          ...
          >
            {connect.charges_enabled ? (
              <p className="text-xs" style={{ color: C.cyan }}>
                💶 Paiements créateur activés — tu reçois{" "}
                {100 - (connect.fee_percent ?? 20)}% des cadeaux
              </p>
            ) : (
              <>
                <p className="text-xs mb-2" style={{ color: C.outline }}>
                  Active les paiements pour recevoir les cadeaux de tes viewers
                  (part créateur {100 - (connect.fee_percent ?? 20)}%).
                </p>
                <button
                  onClick={onboardCreator}
        ...
        <div className="flex flex-col gap-3 w-full max-w-xs">
          ...
          {isHost && (
            <button
              onClick={copyLink}
              className="px-6 py-3 rounded-full font-bold flex items-center justify-center gap-2"
              style={{ background: "#171f33", color: C.cyan }}
            >
              <span className="material-symbols-outlined">link</span>Copier le
              lien
            </button>
          )}
        </div>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(to bottom, rgba(0,0,0,0.5), transparent 18%, transparent 55%, rgba(0,0,0,0.75))",
        }}
      />
          ...
          >
            {FILTERS.map((f, i) => (
              <button
                ...
                style={{
                  background: filterIdx === i ? C.cyan : "transparent",
                  color: filterIdx === i ? C.onPrimary : "#fff",
                }}
              >
                {f.name}
              </button>
      ...
      {flyGifts.map((g) => (
        <div
          ...
          style={{ bottom: 160, animation: "giftPop 3.1s ease-out forwards" }}
        >
        ...
        <button onClick={sendLike} className="flex flex-col items-center gap-1">
          ...
          >
            <span
              ...
              style={{ color: "#f87171", fontVariationSettings: "'FILL' 1" }}
            >
              favorite
            </span>
          </div>
      ...
      <div
        className="absolute left-0 right-16 bottom-0 px-3"
        style={{ paddingBottom: "calc(env(safe-area-inset-bottom) + 12px)" }}
      >
        <div
          className="max-h-[38vh] overflow-y-auto space-y-1.5 mb-2 pr-1"
          style={{
        ...
        >
          <input
            ...
            className="flex-1 bg-transparent outline-none text-sm text-white select-text"
          />
          <button onClick={sendChat} style={{ color: C.cyan }}>
      ...
      {showGifts && (
        ...
        >
          ...
          <div className="grid grid-cols-4 gap-3">
            {GIFTS.map((g) => (
              <button
                key={g.name}
                onClick={() => sendGift(g)}
                className="flex flex-col items-center gap-0.5 py-3 rounded-2xl active:scale-95 transition-transform"
          ...
          >
            {paidGifts
              ? "Paiement sécurisé par Stripe · le créateur reçoit ton soutien"
              : "Cadeaux visuels · configure Stripe pour les cadeaux payants"}
          </p>
        </div>
      )}
