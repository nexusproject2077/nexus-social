...
export default function ClipsSearchPage({ user, setUser }) {
  ...
  const runSearch = useCallback(
    async (query, type, reset = true) => {
      ...
      try {
        ...
        const res = await axios.get(`${API}/clips/search`, {
          params: { q: query, type, skip: sk, limit: 20 },
        });
        const d = res.data || {};
        if (reset) {
          setData({
          ...
          setPage(1);
          setHasMore(
            (d.videos?.length || d.users?.length || d.posts?.length || 0) >= 20,
          );
        } else {
          setData((prev) => ({
            ...prev,
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      if (q.trim()) {
        try {
          const s = await axios.get(`${API}/clips/search/suggest`, {
            params: { q },
          });
    ...
    const io = new IntersectionObserver(
      (entries) => {
        if (
          ...
          hasMore &&
          !loading &&
          ["videos", "users", "posts"].includes(tab)
        ) {
  ...
  const openHashtag = (tagObj) => {
    setQ(tagObj.tag || tagObj);
    setTab("videos");
    inputRef.current?.blur();
  };
  ...
  const Avatar = ({ pic, name, size = "w-11 h-11" }) =>
    pic ? (
      ...
      <div
        className={`${size} rounded-full flex items-center justify-center font-bold flex-shrink-0`}
        style={{
  ...
  const VideoGrid = ({ items }) => (
    <div className="grid grid-cols-3 gap-1.5">
      {items.map((v) => (
        <button
          key={v.id}
          onClick={() => navigate(`/nexus-clips/${v.id}`)}
    ...
    >
      ...
      <div className="min-w-0">
        ...
        <p className="text-xs truncate" style={{ color: C.outline }}>
          {fmt(u.followers_count)} abonnés{u.bio ? ` · ${u.bio}` : ""}
        </p>
      </div>
    </button>
  );
  ...
  const HashRow = ({ t }) => (
    ...
    >
      ...
      >
        <span className="material-symbols-outlined" style={{ color: C.cyan }}>
          tag
        </span>
      </div>
      <div>
        ...
        <p className="text-xs" style={{ color: C.outline }}>
          {t.post_count} clip{t.post_count > 1 ? "s" : ""}
        </p>
      </div>
    </button>
  );
  ...
  const PostRow = ({ p }) => (
    ...
    >
      <Avatar
        pic={p.author_profile_pic}
        name={p.author_username}
        size="w-9 h-9"
      />
      <div className="min-w-0">
        <p className="font-bold text-xs" style={{ color: C.onSurface }}>
          @{p.author_username}
        </p>
        <p className="text-sm line-clamp-2" style={{ color: C.outline }}>
          {p.content}
        </p>
      </div>
    </button>
  ...
  const LiveRow = ({ l }) => (
    <button
      onClick={() => navigate(`/live/${l.room_id}`)}
      className="w-full flex items-center gap-3 py-2.5 text-left"
    >
      <div className="relative">
        <Avatar pic={l.host_profile_pic} name={l.host_username} />
        <span
          className="absolute -bottom-1 left-1/2 -translate-x-1/2 text-[9px] font-black px-1.5 rounded-full"
  ...
  const Empty = ({ label }) => (
    <p className="text-center text-sm py-10" style={{ color: C.outline }}>
      {q.trim() ? `Aucun résultat pour « ${q} »` : label}
    </p>
  );
  ...
  const renderTab = () => {
    ...
    if (tab === "top") {
      const nothing =
        ...
        !data.lives.length;
      if (nothing)
        return <Empty label="Cherche des clips, comptes ou #hashtags" />;
      return (
        <div className="space-y-5">
          ...
          {data.hashtags.length > 0 && (
            <div>
              ...
              {data.hashtags.slice(0, 5).map((t) => (
                <HashRow key={t.tag} t={t} />
              ))}
            </div>
          )}
    ...
    if (tab === "users")
      return data.users.length ? (
        ...
        <Empty label="Cherche des comptes" />
      );
    if (tab === "posts")
      return data.posts.length ? (
        ...
        <Empty label="Cherche des posts" />
      );
    if (tab === "hashtags")
      return data.hashtags.length ? (
        <div>
          {data.hashtags.map((t) => (
            <HashRow key={t.tag} t={t} />
          ))}
        ...
        <Empty label="Cherche des hashtags" />
      );
    if (tab === "live")
      return data.lives.length ? (
        <div>
          {data.lives.map((l) => (
            <LiveRow key={l.room_id} l={l} />
          ))}
      ...
      >
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <div
              className="flex-1 flex items-center gap-2 px-3 h-10 rounded-full"
              style={{ background: C.high }}
            >
              <span
              ...
              {suggests.map((s, i) => (
                ...
                >
                  <span
                    ...
                    style={{ color: C.outline }}
                  >
                    {s.type === "user" ? "person" : "tag"}
                  </span>
                  <span className="text-sm" style={{ color: C.onSurface }}>
                    {s.label}
                  </span>
                </button>
              ))}
