...
function ListsPanel({ user }) {
  ...
  const removeMember = async (uid) => {
    try {
      ...
      setDetail((d) => ({
        ...d,
        members: d.members.filter((m) => m.id !== uid),
      }));
      fetchLists();
    } catch {
      toast.error(i18n.t("error"));
  ...
  const addMember = async (u) => {
    try {
      ...
      setDetail((d) =>
        d.members.some((m) => m.id === u.id)
          ? d
          : { ...d, members: [...d.members, u] },
      );
      fetchLists();
    } catch {
      toast.error(i18n.t("error"));
    ...
    const t = setTimeout(async () => {
      try {
        const r = await axios.get(`${API}/search`, {
          params: { q: addQuery, type: "people", limit: 10 },
        });
    ...
    <div className="pt-2">
      ...
      {loading ? (
        ...
        <div className="text-center py-14 px-4" style={{ color: "#859397" }}>
          ...
          <p className="text-sm">
            Aucune liste. Créez-en une pour regrouper des comptes.
          </p>
        </div>
      ) : (
        lists.map((l) => (
          ...
          >
            <div
              ...
              style={{ background: "#171f33", color: "var(--nexus-accent)" }}
            >
              <span className="material-symbols-outlined">group</span>
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-bold truncate" style={{ color: "#dae2fd" }}>
                {l.name}
              </p>
              <p className="text-[12px]" style={{ color: "#859397" }}>
            ...
            <div className="flex -space-x-2">
              {(l.members_preview || []).map((m) => (
                <Ava
                  ...
                  pic={m.profile_pic}
                  name={m.username}
                  size="w-7 h-7 border-2 border-slate-950"
                />
              ))}
            </div>
          </button>
            ...
            <div className="px-4 py-2 flex-shrink-0">
              {!adding ? (
                ...
                >
                  <span
                    className="material-symbols-outlined"
                    style={{ color: "#859397", fontSize: 18 }}
                  >
                    search
                  </span>
                  <input
              ...
              {!adding &&
                (detail.members.length === 0 ? (
                  <p
                    ...
                    style={{ color: "#859397" }}
                  >
                    Liste vide. Ajoutez des membres.
                  </p>
                ) : (
                  detail.members.map((m) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="text-xs font-bold px-3 py-1.5 rounded-full"
                        style={{ background: "#171f33", color: "#f87171" }}
                      >
                        Retirer
                      </button>
                    </div>
...
export default function SearchPage({ user }) {
  ...
  const highlight = (text) => {
    ...
    return parts.map((p, i) =>
      p.toLowerCase() === q.toLowerCase() ? (
        <span key={i} style={{ color: CY, fontWeight: 700 }}>
          {p}
        </span>
      ) : (
        <span key={i}>{p}</span>
  ...
  useEffect(() => {
    axios
      ...
      .then((r) =>
        setTrending(Array.isArray(r.data?.trending) ? r.data.trending : []),
      )
      .catch(() => setTrending([]));
  }, []);
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      skipRef.current = 0;
      runSearch(query, activeType, true);
    }, 300);
    return () => clearTimeout(t);
  ...
  const handlePostUpdate = (up) =>
    setPosts((p) => p.map((x) => (x.id === up.id ? up : x)));
        ...
        >
          <div className="px-4 pt-3 pb-2">
            ...
            >
              <span
                className="material-symbols-outlined"
                style={{ color: "#859397", fontSize: 20 }}
              >
                search
              </span>
              <input
        ...
        <div className="pb-24 lg:pb-8">
          {tab === "lists" ? (
            ...
            <div className="pt-2">
              ...
              {trending.length === 0 ? (
                <p className="px-4 text-sm" style={{ color: "#859397" }}>
                  Aucune tendance pour l'instant. Publiez avec des #hashtags !
                </p>
              ) : (
                trending.map((t, i) => (
                  <button
            ...
            <div className="flex justify-center py-16">
              <div
                ...
                style={{ borderColor: `${CY}33`, borderTopColor: CY }}
              />
            </div>
          ) : (
            <div>
                ...
                >
                  {hashtags.map((h) => (
                    ...
                    >
                      ...
                      <p className="text-[11px]" style={{ color: "#859397" }}>
                        {h.post_count} publication{h.post_count > 1 ? "s" : ""}
                      </p>
                    </button>
                  ))}
                </div>
                  ...
                  {users.map((u) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px]" style={{ color: "#3c494c" }}>
                          {u.followers_count || 0} abonnés
                        </p>
                      </div>
                    </div>
                  ))}
