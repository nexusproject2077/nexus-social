...
export default function TipModal({
  ...
  const payStripe = async () => {
    if (!amountOk) {
      toast.error(t("tipmodal.err_amount"));
      return;
    }
    setLoading(true);
    try {
      ...
      if (res.data?.url) window.location.href = res.data.url;
        ...
        toast.error(t("tipmodal.err_tip_unavailable"));
        setLoading(false);
      }
    } catch (e) {
      toast.error(
        e.response?.data?.detail || t("tipmodal.err_tip_unavailable"),
      );
      setLoading(false);
    }
  };
  ...
  const payPaypal = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${API}/users/${userId}/paypal-tip`, {
        amount_cents: Math.round(value * 100),
      });
      if (res.data?.url) window.location.href = res.data.url;
      else {
        toast.error(t("tipmodal.err_paypal_unavailable"));
        setLoading(false);
      }
    } catch (e) {
      toast.error(
        e.response?.data?.detail || t("tipmodal.err_paypal_unavailable"),
      );
      setLoading(false);
    }
  };
  ...
  const copyCrypto = async () => {
    try {
      ...
      toast.success(t("tipmodal.crypto_copied"));
    } catch {
      toast.info(cryptoWallet);
    }
  };
      ...
      >
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="font-bold text-white">
            {username
              ? t("tipmodal.support_user", { user: username })
              : t("tipmodal.support")}
          </h3>
        </div>
        ...
        {hasAmountMethod && (
          <>
            ...
            <div
              className="flex items-center rounded-xl px-3 mb-3"
              style={{
                background: "#222a3d",
                border: `1px solid ${custom ? "var(--nexus-accent)" : "rgba(255,255,255,0.08)"}`,
              }}
            >
              <input
                type="number"
              ...
              />
              <span className="text-sm font-bold" style={{ color: "#859397" }}>
                €
              </span>
            </div>
            ...
            <div className="space-y-2">
              ...
              {paypalReceivable && (
                ...
                >
                  ...
                  {t("tipmodal.pay_paypal", {
                    amount: amountOk ? `${value} € ` : "",
                  })}
                </button>
              )}
            </div>
            <p className="text-[11px] mt-2 mb-1" style={{ color: "#859397" }}>
              {t("tipmodal.secure_note")}
            </p>
          </>
        )}
        ...
        {((paypalLink && !paypalReceivable) || cryptoWallet) && (
          <>
            ...
            >
              {hasAmountMethod
                ? t("tipmodal.other_methods")
                : t("tipmodal.available_methods")}
            </p>
            <div className="space-y-2">
