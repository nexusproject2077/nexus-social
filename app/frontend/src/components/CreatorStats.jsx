  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch(async () => {
        ...
        try {
          const r = await axios.get(`${API}/users/${userId}/posts`, {
            params: { type: "clip", limit: 50 },
          });
          const posts = r.data?.posts || r.data || [];
          const clips = posts.filter(
            (p) => p.media_type === "video" || p.is_clip,
          );
          const views = clips.reduce(
            (a, p) => a + (p.views || p.views_count || 0),
            0,
          );
          ...
          setS({
            ...
            views,
            likes,
            avg_views: clips.length ? Math.round(views / clips.length) : 0,
          });
        } catch {
          setS(null);
        }
    ...
    <div className="mx-4 mb-3 grid grid-cols-3 gap-2">
      {[
        ...
        >
          ...
          <p className="text-[10px]" style={{ color: "#859397" }}>
            {x.label}
          </p>
        </div>
      ))}
