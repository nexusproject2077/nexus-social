...
export default function CloseFriendsPanel() {
  ...
  const filtered = following.filter(
    (u) => !q || (u.username || "").toLowerCase().includes(q.toLowerCase()),
  );
  ...
  return (
    ...
    >
      ...
      <div className="max-h-56 overflow-y-auto space-y-1">
        {filtered.length === 0 && (
          <p className="text-xs text-center py-4" style={{ color: "#859397" }}>
            {t("close_friends.empty")}
          </p>
        )}
        {filtered.map((u) => {
