...
export default function CallManager({ user }) {
  ...
  const sendSignal = useCallback((toId, signal) => {
    if (!toId) return;
    axios
      .post(`${API}/calls/signal`, { to_user_id: toId, signal })
      .catch(() => {});
  }, []);
  ...
  const startCall = useCallback(
    async ({ userId, username, profilePic, video }) => {
      if (!userId) return;
      if (phase !== "idle") {
        toast.error(t("call_already_in_progress"));
        return;
      }
      try {
  ...
  const rejectCall = useCallback(() => {
    if (peer)
      sendSignal(peer.id, { kind: "reject", call_id: callIdRef.current });
    cleanup();
  ...
  useEffect(() => {
    const onRealtime = async (e) => {
      ...
      const { from_id, from_username, from_profile_pic, signal } =
        data.data || {};
      if (!signal) return;
      ...
      if (signal.kind === "offer") {
        if (phase !== "idle") {
          sendSignal(from_id, { kind: "reject", call_id: signal.call_id });
          return;
        }
        ...
        if (signal.call_id === callIdRef.current) {
          toast(
            signal.kind === "reject" ? t("call_declined") : t("call_ended"),
          );
          cleanup();
        }
      }
        ...
        {phase === "incoming" ? (
          ...
          <>
            <button
              ...
              style={{
                background: muted ? C.cyan : C.high,
                color: muted ? C.onPrimary : C.onSurface,
              }}
            >
            ...
            {withVideo && (
              <button
                ...
                title={cameraOff ? t("enable_camera") : t("disable_camera")}
                className="w-14 h-14 rounded-full flex items-center justify-center active:scale-90 transition"
                style={{
                  background: cameraOff ? C.cyan : C.high,
                  color: cameraOff ? C.onPrimary : C.onSurface,
                }}
              >
                <span className="material-symbols-outlined">
