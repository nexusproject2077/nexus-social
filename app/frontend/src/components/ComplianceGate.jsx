...
export default function ComplianceGate() {
  ...
  useEffect(() => {
    ...
    if (geo.consentScreen) {
      let stored = null;
      try { stored = localStorage.getItem(APPI_CONSENT_KEY); } catch { /* ignore */ }
      setAppiDone(!!stored);
    } else {
      setAppiDone(true);
    }
      ...
      {geo.consentScreen && !appiDone && (
        <div className="fixed inset-0 z-[1001] flex items-end sm:items-center justify-center sm:p-4"
          ...
          <div className="w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl overflow-hidden"
            ...
            <div className="px-6 pt-7 pb-5 text-center" style={{ background: "linear-gradient(135deg,#052b2f,#0d1424)" }}>
              ...
              <h3 className="text-white font-black text-xl">Protection de vos données</h3>
              <p className="text-sm mt-1" style={{ color: "#7fdbe8" }}>
                Loi APPI (個人情報保護法) — transparence & contrôle
              </p>
            </div>
            ...
            <div className="px-6 py-5">
              <ul className="space-y-2.5 mb-5">
                {[
                  ...
                  "Vous pouvez consulter, corriger et exporter vos données à tout moment.",
                  "Aucune revente de vos données personnelles à des tiers.",
                  "Suppression de votre compte et de vos données en un geste, sans friction.",
                ].map((t) => (
                  <li key={t} className="flex items-start gap-2.5">
                    <span className="mt-0.5 w-5 h-5 rounded-lg flex items-center justify-center flex-shrink-0"
                      ...
                      <Check className="w-3 h-3" style={{ color: "#22d3ee" }} strokeWidth={3} />
                    </span>
                    <span className="text-sm" style={{ color: "#dae2fd" }}>{t}</span>
                  </li>
                ))}
              </ul>

              <button onClick={acceptAppi}
                className="w-full py-3 rounded-2xl font-black text-sm active:scale-[0.98] transition-transform"
                style={{ background: "linear-gradient(135deg,#22d3ee,#3b82f6)", color: "#03242b" }}>
      ...
      {geo.readOnly && !roDismissed && (
        <div className="fixed bottom-3 left-1/2 -translate-x-1/2 z-[1000] w-[94%] max-w-xl">
          <div className="flex items-center gap-3 rounded-2xl px-4 py-3 shadow-xl"
            style={{ background: "rgba(15,23,42,0.96)", border: "1px solid #26324a", backdropFilter: "blur(12px)" }}>
            <span className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0"
              style={{ background: "rgba(148,163,184,0.14)" }}>
              <Eye className="w-4 h-4" style={{ color: "#94a3b8" }} />
            </span>
            <p className="flex-1 text-xs sm:text-sm leading-snug" style={{ color: "#cbd5e1" }}>
              {geo.readOnlyMessage ||
                "Mode consultation : la publication et la messagerie sont indisponibles dans votre pays."}
            </p>
            <button onClick={() => setRoDismissed(true)}
