        ...
        <div className="fixed inset-0 z-[1001] flex items-end sm:items-center justify-center sm:p-4"
          ...
          <div className="w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl overflow-hidden"
            ...
            <div className="px-6 pt-7 pb-5 text-center" style={{ background: "linear-gradient(135deg,#052b2f,#0d1424)" }}>
              ...
              <h3 className="text-white font-black text-xl">{i18n.t("compliance.appi_title")}</h3>
            ...
            <div className="px-6 py-5">
              <ul className="space-y-2.5 mb-5">
                {["appi_1","appi_2","appi_3","appi_4"].map((k) => (
                  <li key={k} className="flex items-start gap-2.5">
                    ...
                    <span className="text-sm" style={{ color: "#dae2fd" }}>{i18n.t("compliance."+k)}</span>
                  </li>
                ))}
              </ul>
              ...
              <div className="flex items-center justify-center gap-4 mt-3 text-xs" style={{ color: "#8ea0c4" }}>
                <button onClick={() => { acceptAppi(); goPrivacyCenter(); }}
                  ...
                  <FileText className="w-3.5 h-3.5" /> {i18n.t("compliance.my_data")}
                </button>
                <span style={{ color: "#3a4759" }}>·</span>
                <button onClick={() => { acceptAppi(); goPrivacyCenter(); }}
                  className="inline-flex items-center gap-1.5 hover:underline" style={{ color: "#f7a1a1" }}>
                  <Trash2 className="w-3.5 h-3.5" /> {i18n.t("compliance.delete_account")}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="fixed bottom-3 left-1/2 -translate-x-1/2 z-[1000] w-[94%] max-w-xl">
          <div className="flex items-center gap-3 rounded-2xl px-4 py-3 shadow-xl"
            ...
            <p className="flex-1 text-xs sm:text-sm leading-snug" style={{ color: "#cbd5e1" }}>
              {geo.readOnlyMessage ||
                i18n.t("compliance.readonly_fallback")}
            </p>
