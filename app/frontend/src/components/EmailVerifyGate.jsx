...
export default function EmailVerifyGate({ user, setUser }) {
  ...
  const send = async (silent = false) => {
    ...
    try {
      ...
      if (r.data?.dev_code) toast.message(t("emailverify.demo_code", { code: r.data.dev_code }), { description: t("emailverify.demo_desc") });
      else if (!silent) toast.success(t("emailverify.code_sent", { email: user?.email }));
    } catch (e) {
      if (!silent) toast.error(e.response?.data?.detail || t("emailverify.send_failed"));
  ...
  const confirm = async () => {
    ...
    } catch (e) { toast.error(e.response?.data?.detail || t("emailverify.invalid_code")); }
    ...
    <div className="fixed inset-0 z-[999] flex items-start sm:items-center justify-center p-4 overflow-y-auto"
      ...
      <div className="w-full max-w-sm rounded-3xl p-6 my-auto"
        ...
        <div className="text-center mb-4">
          ...
          <h2 className="font-black text-lg" style={{ color: "#dae2fd" }}>{t("emailverify.title")}</h2>
          <p className="text-sm mt-1" style={{ color: "#859397" }}>
            <Trans i18nKey="emailverify.sent_to" values={{ email: user?.email }} components={{ b: <b style={{ color: "#dae2fd" }} /> }} />
          </p>
        </div>
        <input
          value={code}
        ...
        <button disabled={busy} onClick={confirm} className="w-full py-3 rounded-2xl font-black disabled:opacity-40"
          style={{ background: ACCENT, color: "#00363e" }}>{busy ? t("emailverify.verifying") : t("emailverify.confirm")}</button>
        ...
        <p className="text-center text-[11px] mt-3" style={{ color: "#5b6b8c" }}>
          ...
          <button onClick={logout} className="ml-1 underline" style={{ color: "#859397" }}>{t("emailverify.logout")}</button>
        </p>
      </div>
    </div>
  );
