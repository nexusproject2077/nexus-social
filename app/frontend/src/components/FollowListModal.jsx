...
export default function FollowListModal({
  ...
  const go = (id) => { onClose?.(); navigate(`/profile/${id}`); };
  ...
  const toggleFollow = async (target) => {
    ...
    try {
      if (target.is_following) {
        ...
        setItems((prev) => prev.map((u) => (u.id === target.id ? { ...u, is_following: false } : u)));
      } else {
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: C.surface, border: `1px solid ${C.outline}22`, maxHeight: "80vh" }}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: `${C.outline}22` }}>
          <h2 className="text-lg font-black text-white" style={{ fontFamily: "Space Grotesk, sans-serif" }}>{title}</h2>
          <button onClick={onClose} className="w-9 h-9 rounded-full flex items-center justify-center" style={{ background: C.surfaceHigh }}>
        ...
        <div className="overflow-y-auto flex-1">
          {loading ? (
            <div className="flex justify-center py-12">
              <div className="w-6 h-6 rounded-full border-2 animate-spin" style={{ borderColor: `${C.accent}33`, borderTopColor: C.accent }} />
            </div>
          ) : items.length === 0 ? (
            ...
            items.map((u) => (
              <div key={u.id} className="flex items-center gap-3 px-5 py-3 hover:bg-white/5">
                <button onClick={() => go(u.id)} className="flex items-center gap-3 flex-1 min-w-0 text-left">
                  {u.profile_pic ? (
                    <img src={u.profile_pic} alt={u.username} className="w-11 h-11 rounded-full object-cover flex-shrink-0" />
                  ) : (
                    <div className="w-11 h-11 rounded-full flex items-center justify-center font-bold flex-shrink-0" style={{ background: "linear-gradient(135deg,#22d3ee,#3b82f6)", color: "#00363e" }}>
