...
export default function NexusAIChat({ onClose }) {
  ...
  return (
    ...
    >
        ...
        <div className="min-w-0">
          ...
          >
            ...
            <span
              ...
              style={{ color: C.accent, fontVariationSettings: "'FILL' 1" }}
            >
              verified
            </span>
          </p>
          <p className="text-[11px]" style={{ color: C.muted }}>
            {i18n.t("aichat.subtitle")}
          </p>
        </div>
      </div>
      ...
      <div
        ref={scrollRef}
        className="flex-1 min-h-0 overflow-y-auto px-3 py-4 space-y-2.5"
      >
        {messages.map((m, i) => (
          <div
            key={i}
          ...
          >
            <div
              className="max-w-[82%] px-3.5 py-2.5 text-sm whitespace-pre-wrap break-words"
              style={
                m.role === "user"
                  ? {
                      background: `linear-gradient(135deg, ${C.accent}, #3b82f6)`,
        ...
        {sending && (
          <div className="flex justify-start">
            <div
              ...
              style={{ background: C.high, borderRadius: "18px 18px 18px 4px" }}
            >
              {[0, 1, 2].map((d) => (
                <span
                  key={d}
      ...
      {/* Saisie */}
      <div
        className="flex items-end gap-2 px-3 pt-2 flex-shrink-0"
        style={{
