...
export default function SmartNotifCard({ user }) {
  ...
  const [prefs, setPrefs] = useState(() => ({ ...DEFAULTS, ...(user?.smart_notif_prefs || {}) }));
  ...
  const toggle = async (key) => {
    ...
    try {
      ...
      try {
        await axios.put(`${API}/users/me/preferences`, { smart_notif_prefs: next });
        toast.success(t("smart_notif.saved"));
      } catch {
  ...
  return (
    <div className="rounded-2xl p-4" style={{ background: "#131b2e", border: "1px solid rgba(255,255,255,0.06)" }}>
      <h3 className="text-sm font-bold text-white mb-1">{t("smart_notif.title")}</h3>
      <p className="text-[11px] mb-3" style={{ color: "#859397" }}>{t("smart_notif.sub")}</p>
        ...
        {rows.map((r) => (
          ...
          >
            ...
            <span
              ...
              style={{ background: prefs[r.key] ? "#22d3ee" : "rgba(255,255,255,0.15)" }}
            >
              <span
