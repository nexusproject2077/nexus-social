...
const FOOT_SECTIONS = [
  {
    key: "europe",
    code: "eu",
    label: "Compétitions européennes",
    leagues: [
      ["uefa.champions", "Ligue des Champions"],
      ["uefa.europa", "Ligue Europa"],
      ["uefa.europa.conf", "Ligue Conférence"],
      ["uefa.super_cup", "Supercoupe d'Europe"],
      ["uefa.wchampions", "Ligue des Champions (F)"],
  ...
  },
  {
    ...
    leagues: [
      ["por.1", "Liga Portugal"],
      ["por.taca.portugal", "Coupe du Portugal"],
    ],
    nations: [],
  },
  {
  ...
  },
  {
    ...
    code: "be",
    label: "Belgique",
    leagues: [["bel.1", "Jupiler Pro League"]],
    nations: [],
  },
  {
    key: "ecosse",
  ...
  },
  {
    ...
    label: "Autres Europe",
    leagues: [
      ["aut.1", "Bundesliga Autriche"],
      ["gre.1", "Super League Grèce"],
      ["den.1", "Superliga Danemark"],
      ["nor.1", "Eliteserien Norvège"],
      ["swe.1", "Allsvenskan Suède"],
      ["rus.1", "Premier League Russie"],
  ...
  },
  {
    key: "conmebol",
    code: "globe",
    label: "Amérique du Sud / CONMEBOL",
    leagues: [
  ...
  },
  {
    ...
    leagues: [
      ...
      ["fifa.wwc", "Coupe du Monde Féminine"],
      ["uefa.euro", "Euro"],
      ["conmebol.america", "Copa América"],
      ["caf.nations", "CAN"],
      ["fifa.cwc", "Coupe du Monde des Clubs"],
      ["fifa.olympics", "Jeux Olympiques (H)"],
      ["fifa.w.olympics", "Jeux Olympiques (F)"],
      ["fifa.friendly", "Matchs amicaux"],
    ],
    nations: [],
  },
];
...
function WeatherIcon({ cond, isDay = true, size = 56 }) {
  ...
  const Cloud = (dy = 0) => (
    <path
      d={`M14,${30 + dy} a6,6 0 0 1 1,-11.9 a8.5,8.5 0 0 1 16.4,2.4 a5.5,5.5 0 0 1 -1.4,10.8 z`}
    />
  );
  const Moon = <path d="M31,26 a11,11 0 1 1 -9,-17 a9,9 0 0 0 9,17 z" />;
  const drops = (ys) =>
    ys.map((x, i) => <line key={i} x1={x} y1={33} x2={x - 2} y2={39} />);
  const snow = (xs) =>
    xs.map((x, i) => (
      <g key={i}>
        <line x1={x} y1={34} x2={x} y2={40} />
        <line x1={x - 3} y1={37} x2={x + 3} y2={37} />
      </g>
    ));
      ...
      return (
        <svg {...p}>
          {isDay ? (
            ...
            <path d="M34,20 a7,7 0 1 1 -6,-11 a6,6 0 0 0 6,11 z" />
          )}
          {Cloud(2)}
        </svg>
      );
    case "cloudy":
      return (
        <svg {...p}>
          {Cloud(0)}
          <path
            d="M20,36 a5,5 0 0 1 0.8,-9.9 a7,7 0 0 1 13.5,2 a4.6,4.6 0 0 1 -1.2,8.9 z"
            opacity="0.5"
          />
        </svg>
      );
...
function StackEditor({ order, smartRotate, onChange, onClose }) {
  ...
  const startDrag = (i, e) => {
    ...
    const move = (ev) => {
      ...
      const hover = Math.max(
        0,
        Math.min(list.length - 1, i + Math.round(dy / ROW_H)),
      );
  ...
  const rowTransform = (i) => {
    ...
    if (drag.index < drag.hover && i > drag.index && i <= drag.hover)
  ...
  const onRowTouchStart = (id, e) => {
    sw.current = { id, x: e.touches[0].clientX };
  };
  ...
  return (
    ...
    >
      <div
        className="w-full sm:max-w-md rounded-t-3xl px-5 overflow-y-auto overscroll-contain no-scrollbar"
        ...
        >
          <div className="flex items-center gap-3">
            <span
              ...
              style={{ color: smart ? NEON : "#8b96a8" }}
            >
              auto_mode
            </span>
            <div className="text-left">
              <p className="text-sm font-bold text-white">
                {t("stack.smart_rotate")}
              </p>
              <p className="text-[11px]" style={{ color: "#859397" }}>
                {t("stack.smart_rotate_hint")}
              </p>
            </div>
          </div>
          <span
          ...
          {list.map((id, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Corps de la ligne */}
                <div
                  className="absolute inset-x-0 top-1 bottom-1 flex items-center gap-3 px-3 rounded-2xl"
                  style={{
                    background: dragging ? "#232c40" : "#1a2234",
                    boxShadow: dragging ? "0 8px 22px rgba(0,0,0,0.5)" : "none",
                    transform: `translateX(${isSwiped ? -72 : 0}px)`,
                    transition: "transform 0.2s ease, background 0.15s",
                  }}
                  onTouchStart={(e) => onRowTouchStart(id, e)}
                  onTouchMove={(e) => onRowTouchMove(id, e)}
...
function WidgetConfig({
  ...
  const toggleAsset = (id) =>
    setAssets((p) => (p.includes(id) ? p.filter((x) => x !== id) : [...p, id]));
  ...
  const searchCity = async () => {
    ...
    try {
      ...
      const r = await fetch(
        `https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(term)}&count=6&language=fr&format=json`,
      );
  ...
  const save = async () => {
    ...
    try {
      ...
      else if (widgetId === "finance") {
        if (assets.length) await onSaveFinance(assets);
  ...
  return (
    ...
    >
      <div
        className="w-full sm:max-w-md rounded-t-3xl p-5 overflow-y-auto overscroll-contain no-scrollbar"
        onTouchMove={(e) => e.stopPropagation()}
        style={{
          background: "#0d1424",
          border: "1px solid rgba(255,255,255,0.08)",
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-4">
          <span
            className="material-symbols-outlined"
            style={{ color: WIDGETS[widgetId]?.color }}
          >
            {WIDGETS[widgetId]?.icon}
          </span>
        ...
        {widgetId === "football" && (
          <div className="max-h-[52vh] overflow-y-auto no-scrollbar">
            ...
            <div
              className="flex items-center gap-2 px-4 py-2.5 rounded-full mb-3"
              style={{
                background: "#0b1220",
                border: "1px solid rgba(255,255,255,0.08)",
              }}
            >
              <span
                ...
                style={{ color: "#6b7686", fontSize: 20 }}
              >
                search
              </span>
              <input
              ...
              {teamQ && (
                <button
                  ...
                  aria-label={t("stack.erase")}
                >
                  <span
                    className="material-symbols-outlined"
                    style={{ color: "#6b7686", fontSize: 18 }}
                  >
                    close
                  </span>
                </button>
            ...
            {teamQ.trim().length >= 2 && (
              <div className="mb-3">
                {teamSearching && teamResults.length === 0 ? (
                  <p className="text-xs px-1 py-2" style={{ color: "#6b7686" }}>
                    {t("stack.searching")}
                  </p>
            ...
            {Object.entries(FINANCE_CATALOG).map(([id, a]) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold"
                  style={{
                    background: on ? "#a78bfa22" : "#1a2234",
                    color: on ? "#c4b5fd" : "#8b96a8",
                    border: `1px solid ${on ? "#a78bfa66" : "rgba(255,255,255,0.08)"}`,
                  }}
                >
                  <span className="material-symbols-outlined text-base">
          ...
          <div>
            <div className="flex gap-2">
              ...
              <button
                ...
                style={{ background: "#232c40", color: "#c7d0e0" }}
              >
            ...
            {picked && (
              ...
              >
                ...
                <button
                  ...
                  style={{ color: "#8b96a8" }}
                >
                  {t("stack.auto_location")}
                </button>
              </div>
            )}
            <div className="mt-2 max-h-[34vh] overflow-y-auto no-scrollbar">
              {searching && (
                <p className="text-xs py-2" style={{ color: "#6b7686" }}>
                  {t("stack.searching")}
                </p>
              )}
              {results.map((r) => (
                <button
                  ...
                  onClick={() => {
                    setPicked({
                      name: [r.name, r.admin1, r.country_code]
                        .filter(Boolean)
                        .join(", "),
                      lat: r.latitude,
                      lon: r.longitude,
...
export default function WidgetStack({ user, setUser }) {
  ...
  const configOrder =
    Array.isArray(cfg.order) && cfg.order.length ? cfg.order : DEFAULT_ORDER;
  const financeAssets =
    Array.isArray(cfg.finance_assets) && cfg.finance_assets.length
      ? cfg.finance_assets
      : DEFAULT_FINANCE_ASSETS;
  const weatherCity =
    cfg.weather_city && typeof cfg.weather_city === "object"
      ? cfg.weather_city
      : null;
  ...
  const [astroSign, setAstroSign] = useState(() => {
    try {
      return localStorage.getItem("nexus_astro_sign") || "";
    } catch {
      return "";
    }
  });
  ...
  const load = useCallback(() => {
    ...
    const favP = axios
      ...
      .then((r) => r.data || {})
      .catch(() => ({}));
    const espnP = fetchLiveScoresFromEspn({
      foot: showFoot,
      mma: showMma,
    }).catch(() => []);
  ...
  useEffect(() => {
    axios
      .get(`${API}/trending/hashtags?limit=6`)
      .then((r) =>
        setTrends(Array.isArray(r.data?.trending) ? r.data.trending : []),
      )
      .catch(() => {});
    let alive = true,
      timer = null;
  ...
  useEffect(() => {
    ...
    const fetchW = (lat, lon) => {
      axios
        .get(`${API}/weather`, { params: { lat, lon } })
        .then((r) => {
          if (alive) setWeather(r.data?.weather || null);
        })
        .catch(() => {});
    };
    const opts = {
    ...
    const run = () => {
      if (weatherCity)
        fetchW(weatherCity.lat, weatherCity.lon); // ville forcée
  ...
  useEffect(() => {
    ...
    const loadFin = () =>
      axios
        .get(`${API}/finance`, { params: { ids: financeKey } })
        .then((r) => {
          if (alive)
            setFinance(Array.isArray(r.data?.assets) ? r.data.assets : []);
        })
        .catch(() => {});
    loadFin();
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((r) => {
        if (alive) setProfileViews(r.data || { count: 0, visitors: [] });
      })
      .catch(() => {
        if (alive) setProfileViews({ count: 0, visitors: [] });
      });
    return () => {
      alive = false;
    };
  ...
  useEffect(() => {
    ...
    Promise.all([
      axios
        ...
        .catch(() => null),
      axios
        .get(`${API}/analytics/me/trends`, { params: { days: 14 } })
        .then((r) => r.data)
        .catch(() => null),
    ]).then(([stats, trend]) => {
      if (alive)
        setAnalytics({ stats, trend: Array.isArray(trend) ? trend : [] });
    });
    return () => {
      alive = false;
    };
  ...
  const applyConfig = (patch) => {
    setUser?.((prev) => {
      if (!prev) return prev;
      const cur = prev.widget_stack_config || {
        smart_rotate: true,
        order: DEFAULT_ORDER,
      };
      return {
        ...
        widget_stack_config: {
          ...
          finance_assets:
            patch.finance_assets ??
            (cur.finance_assets || DEFAULT_FINANCE_ASSETS),
          weather_city:
            "weather_city" in patch
              ? patch.weather_city
              : (cur.weather_city ?? null),
        },
      };
    });
  ...
  const footBase = displayMatches(
    matches.filter((m) => m.sport !== "mma"),
    favL,
    favT,
  );
  const footItems = footBase.length ? footBase : demoOn ? DEMO_FOOT : [];
  const mmaBase = displayMatches(
    matches.filter((m) => m.sport === "mma"),
    favL,
    favT,
  );
  ...
  const isFavOrL1 = (m) =>
    favL.has(m.league_slug) ||
    favT.has(m.home_id) ||
    favT.has(m.away_id) ||
    m.league_slug === "fra.1";
  const startsWithin2h = (m) => {
  ...
  const imminentFavPre = footItems.some(
    (m) => m.state === "pre" && isFavOrL1(m) && startsWithin2h(m),
  );
  const financeSwing = finance.some((a) => Math.abs(a.change_24h || 0) >= 5);
  const avail = {
    football: showFoot && footItems.length > 0,
    mma: showMma && mmaItems.length > 0,
    weather: !!weather,
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      ...
      if (!initRef.current) {
        ...
        if (
          morning &&
          wantWeatherRef.current &&
          !ps.includes("weather") &&
          Date.now() - mountedAtRef.current < 5000
        )
          return;
  ...
  const openWidgetMenu = (id) => {
    ...
    if (id === "finance" && !isPremium) {
      setPremiumPitch("Personnalise les cryptos suivies avec Nexus Premium.");
      return;
    }
    if (CONFIGURABLE[id]) setConfigWidget(id);
    ...
    return (
      <div className="flex items-center justify-between mb-1.5 px-0.5">
        <div className="flex items-center gap-1.5">
          <span
            className="material-symbols-outlined"
            style={{ color: w.color, fontSize: 15 }}
          >
            {w.icon}
          </span>
          <span
        ...
        <button
          onClick={() => openWidgetMenu(id)}
          aria-label={t("stack.customize_widget")}
          className="flex items-center justify-center -m-1 p-1 rounded-lg active:scale-90 transition-transform"
        >
          <span
            className="material-symbols-outlined"
            style={{ color: "#6b7686", fontSize: 16 }}
            title={t("stack.customize")}
          >
            more_horiz
          </span>
        </button>
  ...
  const pageNode = (id) => {
      ...
      return (
        <div className="h-full flex flex-col pl-3 py-2.5">
          ...
          <div
            ...
            style={{ scrollSnapType: "x mandatory", paddingRight: 16 }}
          >
            {footItems.map((m) => (
              <div
                key={m.id}
        ...
        <div className="h-full flex flex-col px-4 py-2.5">
          ...
          <div className="flex-1 flex flex-col justify-center">
            <div className="flex items-baseline gap-1">
              <span
                className="font-extralight leading-none text-white"
                style={{
                  fontSize: 40,
                  textShadow: "0 0 16px rgba(52,211,153,0.25)",
                }}
              >
                {big}
              </span>
              ...
              <span className="text-[11px] ml-1.5" style={{ color: "#8b96a8" }}>
                aujourd'hui
              </span>
            </div>
            {limit > 0 ? (
              <div className="mt-2.5">
                ...
                <p
                  ...
                  style={{ color: over ? "#f87171" : "#6b7686" }}
                >
                  {over
                    ? "Limite du jour atteinte"
                    : `sur ${limit} min · limite quotidienne`}
                </p>
              </div>
            ) : (
              <p className="text-[11px] mt-2" style={{ color: "#6b7686" }}>
                {t("stack.screentime_hint")}
              </p>
            )}
          </div>
        </div>
      ...
      return (
        <div className="h-full flex flex-col px-4 py-2.5">
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar">
            {finance.length === 0 ? (
              <p className="text-xs pt-2" style={{ color: "#6b7686" }}>
                {t("stack.loading_prices")}
              </p>
            ) : (
              finance.map((a) => {
                ...
                return (
                  <div key={a.id} className="flex items-center gap-2 py-1">
                    <span className="text-sm font-black text-white w-12 flex-shrink-0">
                      {a.symbol}
                    </span>
                    <span
                      className="text-sm font-semibold flex-1 text-center tabular-nums truncate"
                      style={{ color: "#dae2fd" }}
                    >
                      {fmtEur(a.price)}
                    </span>
                    <span
                      className="flex-shrink-0 px-2 py-0.5 rounded-full text-[11px] font-bold tabular-nums"
                      style={
                        up
                          ? {
                              background: NEON + "22",
        ...
        <div className="h-full flex flex-col px-4 py-2.5">
          ...
          {weather ? (
            <div className="flex-1 flex items-center gap-3">
              <div
                style={{
                  color: weather.is_day ? "#fbbf24" : "#93c5fd",
                  flexShrink: 0,
                }}
              >
                <WeatherIcon
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start">
                  ...
                  <span
                    className="text-white/70 font-light mt-1"
                    style={{ fontSize: 20 }}
                  >
                    °
                  </span>
            ...
            <div className="flex-1 flex items-center justify-center">
              <p className="text-xs" style={{ color: "#6b7686" }}>
                {t("stack.weather_permission")}
              </p>
            </div>
          )}
        </div>
              ...
              <>
                <div className="flex items-baseline gap-1.5">
                  <span
                    ...
                    style={{
                      fontSize: 34,
                      textShadow: "0 0 14px rgba(244,114,182,0.3)",
                    }}
                  >
                    {count}
                  </span>
                  <span className="text-[12px]" style={{ color: "#8b96a8" }}>
                    visites · 30 j
                  </span>
                </div>
                <div className="flex items-center mt-2.5">
                  {visitors.length ? (
                    visitors.slice(0, 8).map((v, i) =>
                      v.profile_pic ? (
                        <img
                          ...
                          src={v.profile_pic}
                          alt=""
                          className="w-7 h-7 rounded-full object-cover -ml-2 first:ml-0"
                          style={{
                        ...
                        <span
                          key={v.id}
                          className="w-7 h-7 rounded-full -ml-2 first:ml-0 flex items-center justify-center text-[10px] font-bold text-white"
                          style={{
                            background: "#334155",
                            border: "2px solid #0d1424",
                            zIndex: 20 - i,
                    ...
                    <span className="text-[11px]" style={{ color: "#6b7686" }}>
                      {t("stack.no_visits")}
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  {[0, 1, 2, 3].map((i) => (
                    <span
                      key={i}
                      className="w-9 h-9 rounded-full -ml-2.5 first:ml-0"
                      style={{
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-bold text-white flex items-center gap-1">
                    <span
                      className="material-symbols-outlined"
                      style={{ fontSize: 15, color: "#f472b6" }}
                    >
                      lock
                    </span>
                  ...
                  {unlockCTA(
                    "Voyez qui visite votre profil avec Nexus Premium.",
                  )}
                </div>
              </div>
            )}
    ...
    if (id === "ai_analytics") {
      ...
      const reco = !likes.length
        ? "Publiez pour lancer votre croissance."
        : up
          ? "Tendance en hausse : gardez ce rythme de publication."
      ...
      const pts =
        likes.length > 1
          ? likes
              .map(
                (v, i) =>
                  `${(i / (likes.length - 1)) * 100},${28 - (v / maxL) * 24}`,
              )
              .join(" ")
          : "0,16 100,16";
      const chart = (
          ...
          <div className="flex-1 flex flex-col justify-center">
            {isPremium ? (
              ...
              <div>
                ...
                {unlockCTA(
                  "Suivez vos performances et vos conseils IA avec Premium.",
                )}
              </div>
            )}
          </div>
          ...
          {!isPremium ? (
            <div className="flex-1 flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px]" style={{ color: "#8b96a8" }}>
                  {t("astro.hint")}
                </p>
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar">
              <p className="text-[11px] mb-1.5" style={{ color: "#8b96a8" }}>
                {t("astro.choose_sign")}
              </p>
                ...
                {ASTRO_SIGNS.map((s) => (
                  <button
                    ...
                    onClick={() => {
                      setAstroSign(s.key);
                      try {
                        localStorage.setItem("nexus_astro_sign", s.key);
                      } catch {
                        /* ignore */
                      }
                    }}
            ...
            <div className="flex-1 flex flex-col justify-center gap-1.5">
              <div className="flex items-center justify-between">
                <span
                  ...
                  style={{ color: "#c4b5fd" }}
                >
                  {t("astro.sign." + astroSign)}
                </span>
                <button
                  onClick={() => {
                    setAstroSign("");
                    try {
                      localStorage.removeItem("nexus_astro_sign");
                    } catch {
                      /* ignore */
                    }
                  }}
  ...
  return (
    <div className="px-4 pt-1 pb-2">
      ...
      >
        <div
          style={{
            height: STACK_H * pages.length,
            transform: `translateY(-${idx * STACK_H}px)`,
            transition: "transform 0.4s cubic-bezier(0.22,1,0.36,1)",
          }}
        >
          {pages.map((id) => (
            <div key={id} style={{ height: STACK_H }}>
              {pageNode(id)}
            </div>
          ))}
        </div>
      ...
      <PremiumModal
        open={!!premiumPitch}
        feature={premiumPitch}
        onClose={() => setPremiumPitch(null)}
      />
    </div>
  );
}
