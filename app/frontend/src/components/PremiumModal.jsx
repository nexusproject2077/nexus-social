...
const PLANS = {
  annual: {
    price: "34,99 €",
    perKey: "premium.per_year",
    labelKey: "premium.plan_annual",
    noteKey: "premium.note_annual",
    saveKey: "premium.save_badge",
  },
  monthly: {
    price: "3,99 €",
    perKey: "premium.per_month",
    labelKey: "premium.plan_monthly",
    noteKey: "premium.note_monthly",
    saveKey: null,
  },
};
...
export default function PremiumModal({ open, onClose, feature }) {
  ...
  const subscribe = async () => {
    setBusy(true);
    try {
      const r = await axios.post(`${API}/billing/create-checkout-session`, {
        plan,
      });
      if (r.data?.url) {
  ...
  const PlanCard = ({ id }) => {
    ...
    return (
      <button
        ...
        style={{
          background: on ? "rgba(249,217,118,0.10)" : "#141c2e",
          border: `1.5px solid ${on ? "#c8962c" : "rgba(255,255,255,0.08)"}`,
        }}
      >
        {p.saveKey && (
          <span
            className="absolute -top-2.5 left-3 px-2 py-0.5 rounded-full text-[9px] font-black tracking-wide"
            style={{
              background: "#4ade80",
              color: "#04250f",
        ...
        <div className="flex items-center justify-between">
          ...
          <span
            ...
            style={{
              ...
              border: on ? "none" : "1.5px solid #3a4759",
            }}
          >
            {on && (
              <Check
        ...
        <div className="mt-1 flex items-baseline gap-1">
          ...
          <span className="text-[11px]" style={{ color: "#8b96a8" }}>
            {t(p.perKey)}
          </span>
        </div>
        <p className="text-[10px] mt-0.5" style={{ color: "#6b7686" }}>
          {t(p.noteKey)}
        </p>
      </button>
    );
      ...
      >
        ...
        >
          <div
            ...
            style={{
              background: GOLD,
              boxShadow: "0 8px 24px rgba(201,150,44,0.4)",
            }}
          >
        ...
        <div className="px-6 py-5">
          ...
          <button
            ...
            style={{
              background: GOLD,
              color: "#3a2a05",
              boxShadow: "0 8px 24px rgba(201,150,44,0.35)",
            }}
          >
            {busy
