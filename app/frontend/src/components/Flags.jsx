  ...
  switch (code) {
    ...
    case "sco": // Écosse (sautoir de Saint-André)
      return frame(<>
        ...
        <path d="M0 0 L24 16 M24 0 L0 16" stroke="#F7F7F7" strokeWidth="2.6" />
      </>);
    case "sa": // Arabie Saoudite (vert + sabre stylisé)
      return frame(<>
        <rect width="24" height="16" fill="#006C35" />
        <rect x="4.5" y="10.4" width="15" height="0.9" rx="0.45" fill="#F7F7F7" />
        <path d="M4.5 10.85 l-1.6 -0.9 v1.8 z" fill="#F7F7F7" />
        <rect x="7" y="5.2" width="10" height="1.5" rx="0.7" fill="#F7F7F7" opacity="0.92" />
      </>);
    case "us": { // États-Unis (bandes + canton étoilé)
      const stripes = Array.from({ length: 7 }).map((_, i) => (
        <rect key={i} y={(i * 16) / 13 * 2} width="24" height={(16 / 13).toFixed(2)} fill="#B22234" />
      ));
      const dots = [];
      for (let r = 0; r < 3; r++) for (let c = 0; c < 4; c++) {
        dots.push(<circle key={`${r}-${c}`} cx={1.4 + c * 2.1} cy={1.4 + r * 2.4} r="0.4" fill="#F7F7F7" />);
      }
      return frame(<>
        <rect width="24" height="16" fill="#F7F7F7" />
        {stripes}
