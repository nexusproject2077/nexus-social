...
async def register(user: UserIn):
    ...
    # ✅ AJOUT RGPD : Document utilisateur avec champs de conformité
    user_doc = {
        ...
        # Consentements RGPD (par défaut: false, l'utilisateur doit consentir explicitement)
        "consents": {
            "analytics": False,
            "marketing": False,
            "third_party": False,
        ...
        # Suppression programmée (Droit à l'oubli)
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    if user.get("is_blocked", False):
        raise HTTPException(status_code=403, detail="Compte bloqué. Contactez dpo@nexussocial.com")
    
    # ✅ Vérifier si le compte est en attente de suppression
    if user.get("deletion_scheduled", False):
        raise HTTPException(
            status_code=403, 
            detail="Compte en attente de suppression. Vous pouvez annuler dans les Paramètres."
        )
