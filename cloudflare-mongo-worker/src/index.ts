  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (url.pathname === "/api/users/search" && request.method === "GET") {
        const q=String(url.searchParams.get("q")||"").trim();if(!q)return json([],200,request);
        const users=await db.collection("users").find({$or:[{username:{$regex:q,$options:"i"}},{bio:{$regex:q,$options:"i"}}]},{projection:{_id:0,password:0}}).limit(20).toArray(),ids=users.map((u:any)=>u.id).filter(Boolean);
      ...
      if(postRoute){
        ...
        if(action==="post"&&request.method==="DELETE"){if(raw.author_id!==uid)return json({detail:"Not authorized"},403,request);await Promise.all([db.collection("posts").deleteOne({id:postId}),db.collection("likes").deleteMany({post_id:postId}),db.collection("comments").deleteMany({post_id:postId})]);return json({message:"Post deleted successfully"},200,request);}
        if(action==="like"&&request.method==="POST"){const existing=await db.collection("likes").findOne({post_id:postId,user_id:uid});if(existing){await db.collection("likes").deleteOne({_id:existing._id});await db.collection("posts").updateOne({id:postId,likes_count:{$gt:0}},{$inc:{likes_count:-1}});return json({liked:false},200,request);}await db.collection("likes").insertOne({id:crypto.randomUUID(),post_id:postId,user_id:uid,created_at:new Date().toISOString()});await db.collection("posts").updateOne({id:postId},{$inc:{likes_count:1}});if(raw.author_id!==uid)await db.collection("notifications").insertOne({id:crypto.randomUUID(),user_id:raw.author_id,type:"like",from_user_id:uid,from_username:user.username,from_profile_pic:user.profile_pic||null,post_id:postId,read:false,created_at:new Date().toISOString()});return json({liked:true},200,request);}
        ...
        if(action==="comments"&&request.method==="POST"){let b:any;try{b=await request.json();}catch{return json({detail:"Invalid JSON"},400,request);}const content=String(b?.content||"").trim();if(!content)return json({detail:"Comment content is required"},400,request);const comment={id:crypto.randomUUID(),post_id:postId,author_id:uid,author_username:user.username,author_profile_pic:user.profile_pic||null,author_is_verified:Boolean(user.is_verified),content,created_at:new Date().toISOString()};await db.collection("comments").insertOne(comment);await db.collection("posts").updateOne({id:postId},{$inc:{comments_count:1}});if(raw.author_id!==uid)await db.collection("notifications").insertOne({id:crypto.randomUUID(),user_id:raw.author_id,type:"comment",from_user_id:uid,from_username:user.username,from_profile_pic:user.profile_pic||null,post_id:postId,comment_content:content,read:false,created_at:new Date().toISOString()});return json(comment,200,request);}
      }
