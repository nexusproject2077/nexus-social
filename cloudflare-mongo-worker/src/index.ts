...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if(url.pathname==="/api/clips/external"&&request.method==="POST"){let b:any;try{b=await request.json();}catch{return json({detail:"Invalid JSON"},400,request);}const mediaUrl=String(b?.media_url||"");if(!/^https:\/\/res\.cloudinary\.com\//i.test(mediaUrl))return json({detail:"URL média non autorisée"},400,request);const id=crypto.randomUUID(),now=new Date().toISOString(),post:any={id,author_id:uid,author_username:user.username,author_profile_pic:user.profile_pic||null,author_is_verified:Boolean(user.is_verified),content:String(b?.caption||"").trim(),media_type:"video",media_url:mediaUrl,likes_count:0,comments_count:0,shares_count:0,views:0,repost_of:null,eu_blocked:Boolean(b?.eu_blocked),visibility:["public","followers","private"].includes(b?.visibility)?b.visibility:"public",location:String(b?.location||"").slice(0,160),link:/^https?:\/\//i.test(String(b?.link||""))?String(b.link):null,cover_url:typeof b?.cover_url==="string"?b.cover_url:null,allow_comments:b?.allow_comments!==false,allow_remix:Boolean(b?.allow_remix),mature:Boolean(b?.mature),ai_generated:Boolean(b?.ai_generated),is_draft:Boolean(b?.is_draft),duration:Number.isFinite(Number(b?.duration))?Number(b.duration):null,created_at:now};await db.collection("posts").insertOne(post);return json({...post,is_liked:false,is_saved:false},200,request);}
